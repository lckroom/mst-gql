(function (global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('mobx-state-tree'), require('graphql'), require('mobx'), require('graphql-request')) :
	typeof define === 'function' && define.amd ? define(['exports', 'mobx-state-tree', 'graphql', 'mobx', 'graphql-request'], factory) :
	(global = global || self, factory(global.mstGql = {}, global.mobxStateTree, global.graphql, global.mobx, global.graphqlRequest));
}(this, (function (exports, mobxStateTree, graphql, mobx, graphqlRequest) {
	...
	function mergeHelper(store, data) {
	  function merge(data) {
	    ...
	    if (__typename && store.isKnownType(__typename)) {
	      ...
	      let instance = id !== undefined && mobxStateTree.resolveIdentifier(typeDef, store, id);
	...
	class Query {
	  constructor(store, query, variables, options = {}) {
	    ...
	    this.refetch = () => {
	      return Promise.resolve().then(mobx.action(() => {
	        if (!this.loading) {
	          this.fetchResults();
	        }
	...
	const MSTGQLStore = mobxStateTree.types.model("MSTGQLStore", {
	...
	*/
	...
	function MSTGQLRef(targetType) {
	  return mobxStateTree.types.reference(targetType, {
	    get(id, parent) {
	      const node = mobxStateTree.resolveIdentifier(targetType, parent.store || mobxStateTree.getParent(parent).store, id);
	...
	const MSTGQLObject = mobxStateTree.types.model("MSTGQLObject").extend(self => {
	  let store;
	        ...
	        try {
	          return Promise.resolve(storage.getItem(storageKey)).then(function (data) {
	            if (data) {
	              ...
	              const selfType = mobxStateTree.getType(self);
	            ...
	            mobxStateTree.addDisposer(self, mobxStateTree.onSnapshot(self, index_umd.throttle(throttleInterval, data => {
	              if (options.filter) {
...
//# sourceMappingURL=mst-gql.umd.js.map
